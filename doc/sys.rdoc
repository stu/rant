...
* <b>safe_ln(src, dest)</b>
  ...
  This creates a hardlink +dest+ which points to the same file as
  +src+, on platforms that support hardlinks. Simply copies +src+ to
  ...
  Example:
    ...
    sys.safe_ln "main.c", "package/main.c"
        # prints "ln main.c package/main.c" if a hardlink is created
        # prints "cp main.c package/main.c" if is main.c is copied

* <b>ln(src, dest)</b>

  Creates a hardlink +dest+ which points to the same file as +src+. If
  +dest+ is a directory, creates the hardlink <tt>dest/src</tt>.
  ...
  Note::    Not all file systems and operating systems support hard
            links. On those systems, the +ln+ method will raise an
            <tt>Errno::EOPNOTSUPP</tt> exception. Consider using the
            +safe_ln+ method instead.
...
* <b>ln_s(src, dest)</b>
  ...
  Note::    Not all file systems and operating systems support
            symbolic links. On those systems, the +ln_s+ method will
            raise an <tt>Errno::EOPNOTSUPP</tt> exception.
